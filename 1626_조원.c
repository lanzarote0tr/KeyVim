...
void RenderTimer(void) {
    ...
    while (!ThreadFlag) {
        ...
        if (IsCommandMode)
            RenderRange(str, WindowBuffer, Window, (coor){0, Window.y}, (coor){30, Window.y}, CICursor);
        else RenderRange(str, WindowBuffer, Window, (coor){0, Window.y}, (coor){30, Window.y}, Cursor);
        Wait(1000);
        ++CUtime;
    }
    return;
